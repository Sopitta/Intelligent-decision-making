...
try:
    ...
    sys.path.append(glob.glob('/home/sopitta/Desktop/Simulator/CARLA_0.9.11/PythonAPI/carla/dist/carla-*%d.%d-%s.egg' % (
...
from Env.env_continuous_new_s import World, HUD, TensorboardCallback, CheckpointCallback
...
def train_model(env_0, log_dir, log_name, train_num, model_name, train_time, load = False):
    ...
    checkpoint_callback = CheckpointCallback(save_freq=1e5, save_path='./model_checkpoints/{}/'.format(train_num))
    ...
    lr_a = ReduceSchedule(initial_p=1e-3)
    lr_c = ReduceSchedule(initial_p=1e-3)
    #use with lr_schedule.value
    #policy_kwargs = dict(net_arch=[128, 128, 128])
    model = DDPG(policy=MlpPolicy, env=env, verbose=1, tensorboard_log=log_dir,
                 batch_size =32,
                 nb_train_steps = 50, #70
                 nb_rollout_steps = 100, #140
                 nb_eval_steps = 100,
                 ...
                 seed =781)   #20        
    if load:
        model = DDPG.load(load_path = model_name_prev, env = env, tensorboard_log=log_dir,actor_lr=lr_a.value,critic_lr=lr_a.value)
        #model.set_env(env)
...
def evaluate_model(env, model_name, eval_step, log_dir, train_num):
    ...
    for i in range(eval_step):
        ...
        #print(info)
        vel_arr = np.array([action[0],info[0]['v_kmh'],info[0]['a_ms']])
        vel_all.append(vel_arr)
        if done:
            obs = env.reset()
    print('Done evaluation')
    vel_pro_arr = np.vstack(vel_all)
    np.save('vel_pro_ep_'+str(train_num)+'.npy', vel_pro_arr)
