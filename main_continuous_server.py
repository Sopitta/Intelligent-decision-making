...
from Env.env_continuous_server import World, HUD, TensorboardCallback, CheckpointCallback
...
def train_model(env_0, log_dir, log_name, train_num, model_name, train_time, load = False):
    ...
    lr = ReduceSchedule(initial_p=2.5e-3)
    ...
    model = PPO2(policy=MlpPolicy, env=env, verbose=1, tensorboard_log=log_dir,
                 ...
                 lam=0.9,               # [0.9 1] 0.9
                 nminibatches=5,        #!! minibatch [4 4096] con [512 5120], des [32 512] 4
                 noptepochs=5,          #!! epoch [3 30] 4
                 cliprange=0.2,         #!! clipping [0.1 0.3] 0.2
                 seed = 9000)           #6100 6200 6300 6400
    if load:
